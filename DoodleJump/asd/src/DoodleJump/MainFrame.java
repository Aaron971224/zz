...
public class MainFrame extends JFrame implements KeyListener,Runnable {//游戏界面
	//属性
	int MODE,SCORE,TIME=7;//模式，分数,修改数值变小可以减少卡顿(StartFrame,MainFrame)
	...
	MainFrame(int mode,boolean language,boolean music,boolean sound) {//初始化游戏界面(参数为模式)
		...
		this.setIconImage(Toolkit.getDefaultToolkit().getImage(MainFrame.class.getResource("/image/icon.png")));
		addKeyListener(this);
		...
		//背景
		JLabel bgLabel=new JLabel(new ImageIcon(MainFrame.class.getResource(modepath+"BackGround.png")));
		...
		//按钮
		Control=new JButton(new ImageIcon(MainFrame.class.getResource("/image/System/Pause.png")));		
		Back=new JButton(new ImageIcon(MainFrame.class.getResource("/image/System/Pause.png")));
		...
		GameOverLabe1.setIcon(new ImageIcon(MainFrame.class.getResource(languagepath+"System/died.png")));
		GameOverLabe1.setBounds(125,180,200,400);
		DeathLabel1.setIcon(new ImageIcon(MainFrame.class.getResource("/image/Classic Mode/basicline.png")));
		add(DeathLabel1);
		...
		Highscore.setIcon(new ImageIcon(MainFrame.class.getResource(languagepath+"System/high score.png")));
		Highscore.setBounds(40,355,300,150);
		
		Yourname.setOpaque(false);
		Yourname.setVisible(true);
		Yourname.setIcon(new ImageIcon(MainFrame.class.getResource(languagepath+"System/your name.png")));
		Yourname.setBounds(40,300,300,150);
		
		Yourscore.setOpaque(false);
		Yourscore.setVisible(true);
		Yourscore.setIcon(new ImageIcon(MainFrame.class.getResource(languagepath+"System/your score.png")));
		Yourscore.setBounds(40,245,300,150);
		
		GameOverLabe2.setOpaque(false);
		GameOverLabe2.setVisible(true);
		GameOverLabe2.setIcon(new ImageIcon(MainFrame.class.getResource(languagepath+"System/gameover.png")));
		GameOverLabe2.setBounds(70,100,300,150);
		
		
		DeathLabel2.setOpaque(false);
		DeathLabel2.setBounds(0,690*2,450,690);
		DeathLabel2.setIcon(new ImageIcon(MainFrame.class.getResource("/image/Classic Mode/basicline.png")));
		add(DeathLabel2);
		//暂停界面
		PauseLabel=new JLabel();
		PauseLabel.setOpaque(true);
		PauseLabel.setVisible(false);
		PauseLabel.setIcon(new ImageIcon(MainFrame.class.getResource("/image/System/pauselabel.png")));
		...
		Control.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				Score.setBounds(50,5,450,40);
				Control.setBounds(390,5,40,40);
				Control.setIcon(new ImageIcon(MainFrame.class.getResource("/image/System/Pause.png")));
				PauseLabel.setVisible(false);
				PAUSE=false;
				}
				else { 
				...
				Control.setIcon(new ImageIcon(MainFrame.class.getResource("/image/System/Play.png")));
				Back.setIcon(new ImageIcon(MainFrame.class.getResource(languagepath+"Button/menu.png")));
		...
		add(Stan);
		try{
			bing=new MusicPlayer(MainFrame.class.getResource(musicpath+"bing.wav"));//按键声
			jump=new MusicPlayer(MainFrame.class.getResource(musicpath+"jump.wav"));//跳跃
			spring=new MusicPlayer(MainFrame.class.getResource(musicpath+"spring.wav"));//弹簧
			fly=new MusicPlayer(MainFrame.class.getResource(musicpath+"fly.wav"));//飞帽
			rocket=new MusicPlayer(MainFrame.class.getResource(musicpath+"rocket.wav"));//火箭
			boardbreak=new MusicPlayer(MainFrame.class.getResource(musicpath+"break.wav"));//木板破碎
			fly1=new MusicPlayer(MainFrame.class.getResource(musicpath+"Fly1.wav"));//死亡
			move=new MusicPlayer(MainFrame.class.getResource(musicpath+"Move.wav"));//死亡
			monster=new MusicPlayer(MainFrame.class.getResource(musicpath+"Monster.wav"));//死亡
			hole=new MusicPlayer(MainFrame.class.getResource(musicpath+"blackhole.wav"));//死亡
			falldown=new MusicPlayer(MainFrame.class.getResource(musicpath+"falldown.wav"));//死亡
		}catch(Exception e){
		}
		...
		while(true) {
            ...
            for(int i=0;i<30;i++) {//重新绘制木板,道具	
            	...
            	case 4:
    				...
    				if(propt%30<=10)
    					k[i].prop.setIcon(new ImageIcon(MainFrame.class.getResource(modepath+"Fly1.png")));
    				if(propt%30<=20&&propt%30>10)
    					k[i].prop.setIcon(new ImageIcon(MainFrame.class.getResource(modepath+"Fly2.png")));
    				if(propt%30<=30&&propt%30>20)
    				k[i].prop.setIcon(new ImageIcon(MainFrame.class.getResource(modepath+"Fly3.png")));
    				add(k[i].prop);
    				break;
            	case 5:
    				k[i].prop.setLocation(k[i].myPosition[0].x+(int)24*propt/400,k[i].myPosition[0].y-50);
    				if(propt==0)
    					k[i].prop.setIcon(new ImageIcon(MainFrame.class.getResource(modepath+"Move2.png")));
    				if(propt==400)
    					k[i].prop.setIcon(new ImageIcon(MainFrame.class.getResource(modepath+"Move1.png")));
    				add(k[i].prop);
    				break;
            	case 6:
    				k[i].prop.setLocation(k[i].myPosition[0].x,k[i].myPosition[0].y);
            ...
            repaint(0);
				...
				Back.setIcon(new ImageIcon(MainFrame.class.getResource(languagepath+"Button/backmenu.png")));	
				Back.setVisible(true);
